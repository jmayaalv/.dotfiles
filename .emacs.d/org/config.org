...
** atlas
   #+BEGIN_SRC emacs-lisp
     ...
     (defun atlas-migration-name (path client branch)
       ...
       (format "%s/sql/%s/%s-%s.up.sql" path client (format-time-string "%Y%m%dT%H%M%S") branch))
     ...
     (defun atlas-migration (client branch)
       ...
       (interactive "sclient? \nsbranch? ")
       (defvar migration-file-path (atlas-migration-name atlas-home client branch))
       (shell-command (format "git --git-dir=%s/.git checkout %s" atlas-home client))
       (shell-command (format "git --git-dir=%s/.git checkout -b %s" atlas-home branch))
       (switch-to-buffer (generate-new-buffer migration-file-path))
       (write-file migration-file-path)
       (find-file migration-file-path "")
